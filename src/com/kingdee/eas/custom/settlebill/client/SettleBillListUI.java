...
import com.kingdee.eas.custom.settleaccount.settleAccountFacade;
...
public class SettleBillListUI extends AbstractSettleBillListUI
{
	...
	protected FilterInfo getDefaultFilterForQuery(){
		...
		String mask = filterNew.getMaskString();
		...
		filterNew.setMaskString("("+mask+") and (#"+(filterNew.getFilterItems().size()-2)+" or #"+(filterNew.getFilterItems().size()-1)+")");
		return filterNew;
	...
	public void actionEdit_actionPerformed(ActionEvent e) throws Exception
	{
		...
		SettleBillInfo editData = SettleBillFactory.getRemoteInstance().getSettleBillInfo(new ObjectUuidPK(pk));
	...
	throws Exception {
		...
		for(int i=0;i<tblMain.getRowCount();i++){
			if(tblMain.getCell(i, "Fivouchered").getValue().equals(true))
				arrayList.add( (String)tblMain.getCell(i, "id").getValue());
		}
		for(int i=0;i<arrayList.size();i++){
			if(checkIsDeleteVoucher(arrayList.get(i)))
				++num;
		}
		refreshList();
		String result = "校验完成,";
		if(num==0)
			...
			result+="共有"+num+"张更改过凭证标志";
		MsgBox.showInfo(result);
	}
	private boolean checkIsDeleteVoucher(String id) {
		// TODO Auto-generated method stub
		try {
			String str = "select * from T_BOT_Relation where FSrcObjectID='"+id+"'";
			str+=" and FDestEntityID='2652E01E'";
			IRowSet rs = SQLExecutorFactory.getRemoteInstance(str).executeSQL();
			if(rs.size()==0){
				str="update T_SET_SettleBill set FFivouchered=0 where FID='"+id+"'";
