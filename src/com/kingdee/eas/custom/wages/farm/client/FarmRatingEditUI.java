 ...
 * output class name
 */
public class FarmRatingEditUI extends AbstractFarmRatingEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(FarmRatingEditUI.class);
    
    /**
     * output class constructor
     */
    public FarmRatingEditUI() throws Exception
    {
        super();
    }
    /**
		...
		this.getperson.addMouseListener(new MouseListener(){
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(prmtcompany.getValue()==null){
					MsgBox.showWarning("请先选择部门！");
		    		SysUtil.abort();
				}
				WagesFarmOrganInfo info = (WagesFarmOrganInfo) prmtcompany.getValue();
				try {
					rs = getExeQueryRowSet(info);
					kdtEntrys.removeRows();
					PersonInfo p = new PersonInfo();
					while(rs.next()){
						kdtEntrys.addRow();
	 ...
	 * @param info 
	 */
    private IRowSet getExeQueryRowSet(WagesFarmOrganInfo info) throws BOSException {
    	StringBuffer sql = new StringBuffer();
		String sp = "\n";
		sql.append("/*dialect*/").append(sp);
		sql.append("select").append(sp);
		sql.append("t3.FID").append(sp);
		sql.append("from CT_FAR_BasicPostFarm t1").append(sp);
		...
		sql.append("inner join CT_FAR_WagesFarmOrgan t4 on t4.FID = t1.CFFrompositionID").append(sp);
		
		sql.append("where").append(sp);
		sql.append("t4.FID = '"+info.getId().toString()+"'").append(sp);
		System.out.println("获取员工"+sql.toString());
