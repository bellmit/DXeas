...
public class MonthFoodBillEditUI extends AbstractMonthFoodBillEditUI
{
    ...
    public void actionGetPerson_actionPerformed(ActionEvent e) throws Exception
    {
		...
		while(rs.next()){
			...
			//本月计发考核：(考核测算表)奖励金额/达标奖励金额≥70%时，本月计发考核=奖励金额(考核测算表)奖励金额/达标奖励金额＜70%时，本月计发考核=(月工资计算单)考核满额*70%)
			...
			//工资满额 = 基本工资+岗位工资+考核满额+//满勤+补贴+产量工资
...
//    	String id = this.prmtWageOrganization.getData().toString();
		...
		sql.append("t5.入职日期  timeofentry,").append(sp);
		...
		sql.append(") t5 on t5.员工编码 = t3.Fnumber").append(sp);
		...
		//调整金额
		sql.append("left join ( select t2.CFPersonID 员工id,t2.CFPersonnumber 员工编码,sum(t2.CFAmount) 调整金额   from  CT_FOO_AdjustFoodmoney t1").append(sp);
