...
import com.kingdee.eas.custom.wages.PostAllocationFactory;
...
public class PostAllocationControllerBean extends AbstractPostAllocationControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.custom.wages.app.PostAllocationControllerBean");
    @Override
	protected IObjectPK _save(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		// TODO Auto-generated method stub
		PostAllocationInfo info = (PostAllocationInfo) model;
	...
	protected void _audit(Context ctx, IObjectValue model) throws BOSException {
		PostAllocationInfo info = (PostAllocationInfo) model;
		String sql = "";
		PostAllocationEntryInfo infoe;
		PostAllocationEntryCollection infoec = info.getEntrys(); 
		// Ω˚÷π÷ÿ∏¥…Û∫À ∑«Ã·Ωª≤ª…Û∫À
		for(int i = 0;i<info.getEntrys().size();i++){
			infoe =  infoec.get(i);
			System.out.println(infoe.getTogroup().getId());
			sql = "update CT_WAG_BasicPostEntry set FParentID'"+infoe.getTogroup().getId()+"' where CFPersonID = '"+infoe.getPerson().getId()+"'";
...
//		}
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		// TODO Auto-generated method stub
		PostAllocationInfo info = PostAllocationFactory.getLocalInstance(ctx).getPostAllocationInfo(pk);
