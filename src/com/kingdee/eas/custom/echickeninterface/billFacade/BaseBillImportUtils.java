...
import com.kingdee.eas.custom.echickeninterface.common.DataInfoCache;
 ...
 * 
 */
public class BaseBillImportUtils {
	/**
	 * 需要自定义 非公用
	 */
	public BaseBillImportUtils() {
		// TODO Auto-generated constructor stub
	}

	public static List setInfoByNumber(Context ctx, CoreBaseInfo info,
			List errorMsg) throws BOSException, EASBizException {
		return setDataInfoByNumber(ctx, info, errorMsg);
	}

	// 通过编码来给单据赋值，number 转换成 id
	public static List setDataInfoByNumber(Context ctx, CoreBaseInfo info,
			List errorMsg) throws BOSException, EASBizException {
		for (Enumeration keys = info.keys(); keys.hasMoreElements();) {
			...
			else if (obj != null && (obj instanceof DataBaseInfo))
				setDataBaseInfo(ctx, info, errorMsg, key, obj);
			else if (obj != null && (obj instanceof CoreBillBaseInfo))
				setCoreBillInfo(ctx, info, errorMsg, key, obj);
			else if (obj != null && (obj instanceof UserInfo))
				setUserInfo(ctx, info, errorMsg, key, obj);
			else if (obj != null && (obj instanceof IObjectCollection))
				setCollection(ctx, errorMsg, obj);
			else if (obj != null && (obj instanceof SupplierInfo))
				setSupplierInfo(ctx, info, errorMsg, key, obj);// supplierinfo
			else if (obj != null && (obj instanceof WarehouseInfo))
				setWareHouseInfo(ctx, info, errorMsg, key, obj);
			else if(obj!=null && (obj instanceof HenhouseInfo))
				setHenhouseInfo(ctx,info,errorMsg,key,obj);
		}

		return errorMsg;
	}

	private static void setHenhouseInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		HenhouseInfo lInfo=(HenhouseInfo)obj;
		String number=lInfo.getNumber();
		if (lInfo.getId() != null || number == null
				|| number.trim().length() <= 0)
			return;
		String mapKey = (new StringBuilder(String.valueOf(number))).append(
				lInfo.getClass().toString()).toString();
		...
		HenhouseCollection collection=HenhouseFactory.getLocalInstance(ctx).getHenhouseCollection(viewInfo);
		if (collection != null && collection.size() > 0) {
			lInfo = collection.get(0);
			DataInfoCache.getInstance().put(mapKey, lInfo);
		} else {
			lInfo = null;
			errorMsg.add((new StringBuilder("\u7F16\u7801")).append(number)
					.append(" \u4E0D\u662F\u4E00\u4E2A\u6B63\u786E\u7684 ")
					.append(getAliasName(ctx, info, key)).append(" .")
					.toString());
		}
		info.put(key, lInfo);
	}

	public static void setMaterial(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		...
		viewInfo.setFilter(filter);
		viewInfo.setSelector(sic);
		MaterialCollection collection = MaterialFactory.getLocalInstance(ctx)
	...
	// 仓库
	private static void setWareHouseInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		// TODO Auto-generated method stub
		WarehouseInfo lInfo = (WarehouseInfo) obj;
		String number = lInfo.getNumber();
		if (lInfo.getId() != null || number == null
				|| number.trim().length() <= 0)
			return;
		...
		WarehouseCollection warehouseCol = iwarehouse
				.getWarehouseCollection(new StringBuilder(
						"select id,number,name where number='").append(number)
						.append("'").toString());

		if (warehouseCol != null && warehouseCol.size() > 0) {
			lInfo = warehouseCol.get(0);
			DataInfoCache.getInstance().put(mapKey, lInfo);
		} else {
			lInfo = null;
			errorMsg.add((new StringBuilder("\u7F16\u7801")).append(number)
	...
	// 根据供应商编码，添加供应商id到info
	private static void setSupplierInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		// TODO Auto-generated method stub
		SupplierInfo lInfo = (SupplierInfo) obj;
		String number = lInfo.getNumber();
		if (lInfo.getId() != null || number == null
				|| number.trim().length() <= 0)
			return;
		...
		SupplierCollection supplierCol = isupplier
				.getSupplierCollection(new StringBuilder(
						"select id,number,name where number='").append(number)
						.append("'").toString());
		if (supplierCol != null && supplierCol.size() > 0) {
			lInfo = supplierCol.get(0);
			DataInfoCache.getInstance().put(mapKey, lInfo);
		} else {
			lInfo = null;
			errorMsg.add((new StringBuilder("\u7F16\u7801")).append(number)
	 ...
	 * @throws BOSException
	 */
	public static boolean checkBillNumberExist(Context ctx,
			BOSObjectType bosType, String number) throws BOSException {

		ICoreBillBase iCoreBillBase = (ICoreBillBase) BOSObjectFactory
				.createBOSObject(ctx, bosType);

		CoreBillBaseCollection databaseCol = iCoreBillBase
				.getCoreBillBaseCollection((new StringBuilder(
						"select id,number where number='")).append(number)
						.append("'").toString());
	...
	private static String getOql(DataBaseInfo lInfo, String number) {
		...
		if (lInfo instanceof CurrencyInfo)
			sb.append("select id,number,name,precision where number='").append(
					number).append("'");
		else if (lInfo instanceof RowTypeInfo)
			sb
					.append(
							"select id,number,name,isHasMaterialNumber,precision,destinationtype where number='")
					.append(number).append("'");
		else
			sb.append("select id,number,name where number='").append(number)
					.append("'");
		return sb.toString();
	}

	public static void setDataBaseInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		...
		IDataBase iDataBase = (IDataBase) BOSObjectFactory.createBOSObject(ctx,
				lInfo.getBOSType());
	...
	public static void setCoreBillInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		CoreBillBaseInfo lInfo = (CoreBillBaseInfo) obj;
		if (lInfo.getId() != null)
			return;
		ICoreBillBase iCoreBillBase = (ICoreBillBase) BOSObjectFactory
				.createBOSObject(ctx, lInfo.getBOSType());
		String number = lInfo.getNumber();
		CoreBillBaseCollection databaseCol = iCoreBillBase
				.getCoreBillBaseCollection((new StringBuilder(
						"select id,number where number='")).append(number)
	...
	public static void setUserInfo(Context ctx, CoreBaseInfo info,
			List errorMsg, String key, Object obj) throws BOSException {
		...
		UserCollection userCol = iuser.getUserCollection((new StringBuilder(
				"select id,number ,name where number='")).append(number)
				.append("'").toString());
	...
	public static MultiMeasureUnitInfo getMultiMeasureUnit(Context ctx,
			MaterialInfo material, MeasureUnitInfo unit, List errorMsg)
			throws BOSException {
		MultiMeasureUnitInfo mmu = null;
		if (material != null && unit != null) {
			String key = (new StringBuilder(String.valueOf(material.getId()
					.toString()))).append(unit.getId().toString()).toString();
			if (DataInfoCache.getInstance().containsKey(key)) {
				mmu = (MultiMeasureUnitInfo) DataInfoCache.getInstance().get(
						key);
			} else {
				mmu = getMultiMeasureUnitInfo(ctx, material.getId().toString(),
						unit.getId().toString());
				if (mmu != null)
					DataInfoCache.getInstance().put(key, mmu);
				else
					errorMsg
							.add((new StringBuilder("\u8BA1\u91CF\u5355\u4F4D"))
									.append(unit.getNumber())
									.append(
											"\u4E0D\u5B58\u5728\u4E8E\u7269\u6599\u7684")
									.append(material.getNumber())
									.append(
											"\u591A\u8BA1\u91CF\u5355\u4F4D\u7EC4\u91CC\u9762")
									.toString());
			}
	...
	private static MultiMeasureUnitInfo getMultiMeasureUnitInfo(Context ctx,
			String materialId, String unitId) throws BOSException {
		MultiMeasureUnitInfo mmu = null;
		IMultiMeasureUnit imultiUnit = MultiMeasureUnitFactory
				.getLocalInstance(ctx);
		String oql = (new StringBuilder(
				" select material.id, measureUnit.id, qtyPrecision,baseConvsRate where measureUnit.id ='"))
				.append(unitId).append("' and material.id ='").append(
						materialId).append("'").toString();
	...
	public static MaterialCompanyInfoInfo getMaterialCompanyInfo(Context ctx,
			MaterialInfo material, CompanyOrgUnitInfo cou, List errorMsg)
			throws BOSException {
		MaterialCompanyInfoInfo materialcompanyInfo = null;
		if (material != null && cou != null) {
			String key = (new StringBuilder(String.valueOf(material.getId()
					.toString()))).append(cou.getId().toString()).toString();
			if (DataInfoCache.getInstance().containsKey(key)) {
				materialcompanyInfo = (MaterialCompanyInfoInfo) DataInfoCache
						.getInstance().get(key);
			} else {
				materialcompanyInfo = getMaterialCompanyInfo(ctx, material
						.getId().toString(), cou.getId().toString());
	...
	private static MaterialCompanyInfoInfo getMaterialCompanyInfo(Context ctx,
			String materialid, String couID) throws BOSException {
		MaterialCompanyInfoInfo materialcompanyInfo = null;
		EntityViewInfo viewInfo = new EntityViewInfo();
		FilterInfo filter = new FilterInfo();
		filter.getFilterItems().add(
				new FilterItemInfo("mateial", materialid, CompareType.EQUALS));
		filter.getFilterItems().add(
				new FilterItemInfo("company", couID, CompareType.EQUALS));
		viewInfo.setFilter(filter);
		SelectorItemCollection sic = new SelectorItemCollection();
		sic.add(new SelectorItemInfo("standardCost"));
		MaterialCompanyInfoCollection con = MaterialCompanyInfoFactory
				.getLocalInstance(ctx).getMaterialCompanyInfoCollection(
						viewInfo);
	...
	public static CompanyOrgUnitInfo getCompanyInfo(Context ctx,
			OrgUnitInfo orgInfo, int orgType) throws BOSException,
			EASBizException {
		CompanyOrgUnitInfo couInfo = null;
		if (orgInfo == null)
			return null;
		IOrgUnitRelation iOrgRelation = OrgUnitRelationFactory
				.getLocalInstance(ctx);
		OrgUnitCollection orgCollection = iOrgRelation.getToUnit(orgInfo
				.getId().toString(), orgType, 1);
	...
	public static void getbillMetas(Context ctx, String bosType) {
		SimplePropertyCollection properties = MultiLangUtils.getAllProperties(
				ctx, BOSObjectType.create(bosType));
		for (int i = 0; i < properties.size(); i++) {
			SimpleProperty sp = properties.get(i);
			String sline = (new StringBuilder(String.valueOf(sp.getName())))
					.append("; ").append(sp.getAlias()).append("; ").append(
							sp.getString("description")).toString();
			if (sp.isOwnProperty())
				sline = (new StringBuilder(String.valueOf(sline))).append("; ")
						.append(sp.getDataType().getJavaTypeName()).toString();
			else
				sline = (new StringBuilder(String.valueOf(sline))).append("; ")
						.append(sp.getRefTypeInfo()).toString();
	...
	public static void outputBillMetas(Context ctx) {
		...
		System.out
				.println("com.kingdee.eas.scm.sm.pur.app.PurOrderExpenseEntry");
