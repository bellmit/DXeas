 ...
 *
 */
public class PorInWarehsBillEditDep {
//
//	// 编辑界面
...
//	 */
	public void afterOnLoad(final UIParam param){
		...
		final KDTable table=param.getKDTable("entry");
		table.addKDTEditListener(new KDTEditAdapter(){
			//物料带出其简称
			public void editStopped(KDTEditEvent event) {
				if(table.getColumnKey(event.getColIndex()).equals("material")) {
					
					if(table.getCell(event.getRowIndex(),"material")!=null) {
						MaterialInfo minfo;
						try {
							minfo = MaterialFactory.getRemoteInstance().getMaterialInfo(new ObjectUuidPK(table.getCell(event.getRowIndex(),"material").toString()));
							table.getCell(event.getRowIndex(),"materialnick").setValue(minfo.getAlias());
