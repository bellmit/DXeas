 ...
 *
 */
public class SynDataFodderDrawToReturnMaterialReq implements ISynData{
	...
	public String synData(Context ctx, String billId, String entryId,
		...
		//备注
		String FDescription = "饲料领用单同步生产退料单";
		...
		//获取该物料的辅助计量单位的换算关系  
		MeasureUnitInfo assInfo = MeasureUnitFactory.getLocalInstance(ctx).getMeasureUnitInfo(new ObjectUuidPK(((MeasureUnitInfo)matInfo.getAssistUnit()).getId()));
		BigDecimal exchange = assInfo.getCoefficient();
		BigDecimal receiveQty = entryInfo.getReceiveQty().multiply(new BigDecimal("-1"));
		String FAppQty = String.valueOf(receiveQty);
		String exchangeAppQty = String.valueOf(exchange.multiply(receiveQty));
		//申请数量
		...
		//主库存实退数量
		String FStockQty = "0";
		manReturnEntryInfo.setFStockQty(exchangeAppQty);
