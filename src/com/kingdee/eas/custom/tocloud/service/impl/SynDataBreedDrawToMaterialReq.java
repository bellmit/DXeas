...
import com.kingdee.eas.basedata.scm.im.inv.Warehouse;
...
public class SynDataBreedDrawToMaterialReq implements ISynData{
	...
	public String synData(Context ctx, String billId, String entryId,
		...
		//实例化
		BreedSeedReceiveBillInfo breedSeedReceiveBillInfo = BreedSeedReceiveBillFactory.getLocalInstance(ctx).getBreedSeedReceiveBillInfo(new ObjectUuidPK(billId));
		BreedSeedReceiveBillEntryInfo entryInfo = BreedSeedReceiveBillEntryFactory.getLocalInstance(ctx).getBreedSeedReceiveBillEntryInfo(new ObjectUuidPK(entryId));
		//设定日期显示格式
		...
		//日期
		String FDate = breedSeedReceiveBillInfo.getBizDate().toString();
		cloudMaterialReqBean.setFDate(FDate);

		//备注
		String FDescription = "雏苗领用单同步领料出库单";
		cloudMaterialReqBean.setFDescription(FDescription);

		//发料组织
		CompanyOrgUnitInfo storageInfo = CompanyOrgUnitFactory.getLocalInstance(ctx).getCompanyOrgUnitInfo(new ObjectUuidPK(breedSeedReceiveBillInfo.getCompany().getId()));
		...
		//生产车间
		FarmerInfo farmInfo = FarmerFactory.getLocalInstance(ctx).getFarmerInfo(new ObjectUuidPK(breedSeedReceiveBillInfo.getFarmer().getId()));
		f7Bean = new CloudCommonF7Bean();
		...
		//批号
		String type = breedSeedReceiveBillInfo.getSeedSourceType().getValue();
		BatchInfo batchInfo = breedSeedReceiveBillInfo.getBatch();
		...
		if(batchInfo != null){
			...
			if("0".equalsIgnoreCase(type)){
				f7Bean.setFNumber("内采");
			}if("1".equalsIgnoreCase(type)){
				f7Bean.setFNumber("外购");
			}

			cloudMaterialReqEntryBeanInfo.setFLot(f7Bean);
		} else {
			throw new Exception("请维护雏苗领用单批次");
		}
