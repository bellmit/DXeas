...
public class SynDataBadEggToOtherInWare implements ISynData {

	/**
	 * 业务类型是产蛋信息非合格单类型--生成cloud其他入库单(003)
	 */
	@Override
	public String synData(Context ctx, String billId, String entryId,
		...
		StockingBreedDailyEggEntryInfo eggEggInfo = StockingBreedDailyEggEntryFactory.getLocalInstance(ctx).getStockingBreedDailyEggEntryInfo(new ObjectUuidPK(entryId));
		...
		String FStockOrgId = companyInfo.getNumber();
		f7Bean = new CloudCommonF7Bean();
		...
		try {
			...
			while(rs.next()){
				...
				//实收数量
				String FQty = eggEggInfo.getString(srcNum);
				...
				//批号
				StockingBatchInfo batchInfo = StockingBatchFactory.getLocalInstance(ctx).getStockingBatchInfo(new ObjectUuidPK(stockingInfo.getStockingBatch().getId()));
