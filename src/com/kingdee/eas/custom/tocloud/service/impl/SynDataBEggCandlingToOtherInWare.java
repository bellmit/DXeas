...
public class SynDataBEggCandlingToOtherInWare implements ISynData{

	/**
	 * 业务类型是照蛋记录单--生成cloud 其他入库单（007）
	 */
	@Override
	public String synData(Context ctx, String billId, String entryId,
		...
		//实例化
		BEggCandlingBillInfo bEggCandlingBill = BEggCandlingBillFactory.getLocalInstance(ctx).getBEggCandlingBillInfo(new ObjectUuidPK(billId));
		BEggCandlingBillEggEntryInfo entryInfo = BEggCandlingBillEggEntryFactory.getLocalInstance(ctx).getBEggCandlingBillEggEntryInfo(new ObjectUuidPK(entryId));
		...
		//库存组织
		StorageOrgUnitInfo storageInfo = StorageOrgUnitFactory.getLocalInstance(ctx).getStorageOrgUnitInfo(new ObjectUuidPK((bEggCandlingBill.getCU().getId())));
		...
		//日期
		String FDate = bEggCandlingBill.getBizDate().toString();
		...
		//部门（棚舍）
		//		FarmHouseEntryInfo houseInfo = entryInfo.getOutHouse();
		//		if(houseInfo != null){
		...
		String deptionNum = "ZQ08";
		f7Bean = new CloudCommonF7Bean();
		f7Bean.setFNumber(deptionNum);
		...
		//这里需要根据导入哪些蛋对照蛋记录单的分录进行遍历



		String sql = "select  t2.CFSrcNumber, t2.CFSrcName ,t2.CFTargetNumber,t2.CFTargetName  from CT_CLD_BaseDataRel t1 " +
		" inner join CT_CLD_BaseDataRelEntry t2 on t2.FParentID=t1.fid inner join CT_CLD_BaseDataType t3 on t3.fid=t1.CFBaseDataTypeID where t3.FNumber='008' ";
		String srcNum = null;
		String srcName = null;
		...
		while(rs.next()){
			...
			//实收数量
			String FQty = String.valueOf(entryInfo.getString(srcNum));
			...
			if(batchInfo != null){
				StockingBatchInfo batchInfo1 = StockingBatchFactory.getLocalInstance(ctx).getStockingBatchInfo(new ObjectUuidPK(batchInfo.getId()));
				String batchNum = batchInfo1.getNumber().substring(0, 8);
				f7Bean = new CloudCommonF7Bean();
				f7Bean.setFNumber(batchNum);
				otherInWarehsBillRecEntryBean.setFLOT(f7Bean);
			} else {
				throw new Exception("请维护照蛋记录单分录的种禽批次");
			}
