...
public class BotpCommUtils {
	 ...
	 */
	public static void isExitBTPBill(Context ctx, IObjectPK pk, CoreBillBaseInfo coreBillInfo) throws BOSException,EASBizException {
		...
		sql.append(" where FSrcObjectID=? and FDestEntityID!=?");
		sql.append(" ");
		IRowSet rs = DbUtil.executeQuery(ctx, sql.toString(),new Object[]{coreBillInfo.getString("id"),MaterialRequest_BOSTYPE});
		try {
			if(rs.next()) {
				throw new BillBaseException(new NumericExceptionSubItem("001","单据存在下游单据")); 
			}
