 ...
 * 
 */
public class ImportCollMaterialReqBill implements IImport2EAS {
...
//			Date date3 = sdf.parse(endDate + " 00:00:00");
			...
			//领料出库单导入
			ArrayList<String> tempBillIDList = new ArrayList<String>();
			...
			HashMap<String, BigDecimal> llqtyMap = new HashMap<String, BigDecimal>();// 理论数量
			BigDecimal qty = BigDecimal.ZERO;// 同一物料的累计数量
			BigDecimal llqty = BigDecimal.ZERO;// 同一物料的累计ll数量
				
			// 获取中控数据
			PreparedStatement ptm = conn.prepareStatement(getQueryStr(),ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			ptm.setTimestamp(1, new Timestamp(date.getTime()));
			ptm.setTimestamp(2, new Timestamp(date3.getTime()));
			ptm.setString(3, TempClass);
...
//				}
				...
				if (!orderBillIDKeep.equals(orderBillID)||rs.getRow() == (count)) {
					...
					if(sourceBillIDs!=null&&sourceBillIDs.size()>0&&!qtyMap.isEmpty()){
						exeImportByBotp(ctx, conn, sourceBillIDs, qtyMap,llqtyMap,date);
						//updateHadImportFlag(conn, tempBillIDList);
						tempBillIDList.add(orderBillID);
	 ...
	 */
	private void exeImportByBotp(Context ctx, Connection conn, HashSet<String> sourceBillIDs, HashMap<String, BigDecimal> qtyMap,HashMap<String, BigDecimal> llqtyMap, Date date) throws BOSException, ClassNotFoundException, EASBizException {
...
//			info = new MaterialReqBillInfo();
		...
		info.setSupplyStoreOrgUnit(ContextUtil.getCurrentStorageUnit(ctx));
		isCoust = true;
		...
		MaterialReqBillEntryCollection infoentrycc = info.getEntry();
		IMaterial ims =null;
		if(ctx!=null){
		...
		//班组
		//info.put("groupnumber", sourceBillID);
...
//				warehouse = EASWUtils.getDefaultLLWareHouse(ctx, info.getStorageOrgUnit().getString("id"));//ContextUtil.getCurrentOrgUnit(ctx).getId().toString()
		...
		try {
			...
			for(Map.Entry<String,BigDecimal> entry:qtyMap.entrySet()) {
				...
				entryInfo.setQty(qtyMap.get(entryInfo.getMaterial().getNumber()));
				entryInfo.setBaseQty(entryInfo.getQty());
				//entryInfo.put("llqty", llqtyMap.get(entryInfo.getMaterial().getNumber()));
				entryInfo.setUnit(mInfo.getBaseUnit());
				entryInfo.setBaseUnit(mInfo.getBaseUnit());
				WarehouseInfo warehouse=EASWUtils.getDefaultByMaterial(ctx,info.getStorageOrgUnit().getString("id") ,	entryInfo.getMaterial().getId().toString());//ContextUtil.getCurrentOrgUnit(ctx).getId().toString()
				if (warehouse == null) {
				    warehouse = EASWUtils.getDefaultLLWareHouse(ctx, info.getStorageOrgUnit().getString("id"));//ContextUtil.getCurrentOrgUnit(ctx).getId().toString();
				}
				entryInfo.setWarehouse(warehouse);// 物料默认仓库
...
//	}
	...
	public String getQueryStr() {
		// TODO Auto-generated method stub
		StringBuffer sql = new StringBuffer();
		sql.append(" select sum(recipeWeight) recipeWeight,operator,materialName,materialNum,sum(actualWeight) actualWeight,workshop,company ");// fplid,plid,plno,ylid,ylzl,llyl,gc,mz,pz,zsd,
		...
		sql.append(" and operator like ?");// 领料日期
		sql.append(" group by operator,materialName,materialNum,workshop,company");// 领料日期
		sql.append(" order by operator");
		return sql.toString();
	}
