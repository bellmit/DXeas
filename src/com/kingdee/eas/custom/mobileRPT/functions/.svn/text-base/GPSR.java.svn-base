...
public class GPSR implements IDownload {
	...
	public JSONArray downloadData(Context ctx, JSONObject queryJSON)
		...
		sql.append("SELECT rownum rn,materialName,model,saleQty,saleAmount,saleIncome from( ");
		sql.append("select ");
...
//		sql.append("INNER JOIN T_ORG_Storage t3 on t1.FStorageOrgUnitID=t3.FID ");
		...
		sql.append("WHERE ");
		sql.append("TO_CHAR(t1.FBIZDATE,'yyyy-MM-dd')>='").append(beginDate).append("' ");
		...
		sql.append("GROUP BY ");
		sql.append("t4.FName_l2,t4.FModel ");
