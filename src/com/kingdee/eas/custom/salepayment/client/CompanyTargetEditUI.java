 ...
 * output class name
 */
public class CompanyTargetEditUI extends AbstractCompanyTargetEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(CompanyTargetEditUI.class);
    
    /**
     * output class constructor
     */
    public CompanyTargetEditUI() throws Exception
    {
        super();
    ...
    public void setDataObject(IObjectValue dataObject) 
    {
        ...
        if(STATUS_ADDNEW.equals(getOprtState())) {
            editData.put("treeid",(com.kingdee.eas.custom.salepayment.CompanyTargetTreeInfo)getUIContext().get(UIContext.PARENTNODE));
    ...
    public void beforeStoreFields(ActionEvent e) throws Exception{
    	...
    	if(selType != null){
    		Quarter selQuarter = (Quarter) this.quater.getSelectedItem();
    		// 校验 季度 是否与 指标类型匹配
    		if(selQuarter != null){
...
//		objectValue.setCompany(SysContext.getSysContext().getCurrentFIUnit());
     ...
     */
	@Override
	public void kdtAreaEntry_Changed(int rowIndex, int colIndex)
			throws Exception {
		if(colIndex == this.kdtAreaEntry.getColumnIndex("areaProfitTarget")){
			updateAreaProfitSum();
		}
	}
	...
	private void updateAreaProfitSum() {
		Integer profitSum = new Integer(0);
		for(int i = 0; i < this.kdtAreaEntry.getRowCount(); i++){
			Integer areaProfit = (Integer) this.kdtAreaEntry.getCell(i, "areaProfitTarget").getValue();
			if(areaProfit != null){
		...
		// 设置区域合计
		this.txtareaProfitSum.setValue(profitSum);
	}
