...
import com.kingdee.eas.custom.salepayment.CustomerSaleTargetFactory;
import com.kingdee.eas.custom.salepayment.CustomerSaleTargetInfo;
import com.kingdee.eas.scm.common.BillBaseStatusEnum;
import com.kingdee.eas.wlhlcomm.AppCommon;
import com.kingdee.util.NumericExceptionSubItem;

public class CustomerSaleTargetControllerBean extends AbstractCustomerSaleTargetControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.custom.salepayment.app.CustomerSaleTargetControllerBean");
	...
	protected IObjectPK _save(Context ctx, IObjectValue model)
    	...
    	CustomerSaleTargetInfo billInfo = (CustomerSaleTargetInfo)model;
    	if(billInfo != null  && 
...
//    		throw new BOSException("只能保存新增或保存状态的单据");
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException,
			EASBizException {
		CustomerSaleTargetInfo billInfo = CustomerSaleTargetFactory.getLocalInstance(ctx).getCustomerSaleTargetInfo(pk);
    	if(billInfo != null  && 
    			(BillBaseStatusEnum.NULL.equals(billInfo.getBaseStatus())  || 
    					BillBaseStatusEnum.ADD.equals(billInfo.getBaseStatus()) ||
    						BillBaseStatusEnum.TEMPORARILYSAVED.equals(billInfo.getBaseStatus()) ||
...
//    		throw new BOSException("只能删除新增或保存状态的单据");
	...
	protected void _audit(Context ctx, IObjectValue model) throws BOSException {
		...
		if(billInfo != null && billInfo.getBaseStatus() != null && BillBaseStatusEnum.SUBMITED.equals(billInfo.getBaseStatus())){
			...
			// 更新单据状态
			AppCommon.updateBaseDataModelStatus(ctx, "CT_SP_CustomerSaleTarget", billInfo.getId().toString(), BillBaseStatusEnum.AUDITED);
	...
	protected void _unAudit(Context ctx, IObjectValue model)
		...
		if(billInfo != null && billInfo.getBaseStatus() != null && BillBaseStatusEnum.AUDITED.equals(billInfo.getBaseStatus())){
			...
			// 更新单据状态
			AppCommon.updateBaseDataModelStatus(ctx, "CT_SP_CustomerSaleTarget", billInfo.getId().toString(), BillBaseStatusEnum.TEMPORARILYSAVED);
