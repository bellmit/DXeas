 ...
 * output class name
 */
public class PackingConversionEditUI extends AbstractPackingConversionEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(PackingConversionEditUI.class);
    
    /**
     * output class constructor
     */
    public PackingConversionEditUI() throws Exception
    {
        super();
    }
    /**
	...
	public void onLoad() throws Exception {
        ...
        this.prmtmaterialProduct.setEditFormat("$name$");		
        this.prmtmaterialProduct.setCommitFormat("$name$");
        this.prmtmaterialPacking.setDisplayFormat("$name$");		
        this.prmtmaterialPacking.setEditFormat("$name$");		
        this.prmtmaterialPacking.setCommitFormat("$name$");
		 /*EntityViewInfo ev1=new EntityViewInfo();
		FilterInfo filter1=new FilterInfo();
		filter1.getFilterItems().add(new FilterItemInfo("materialProduct.id","select FID from T_BD_Material where FNumber like '01%'",CompareType.INNER));
		ev1.setFilter(filter1);
		prmtmaterialProduct.setEntityViewInfo(ev1);
		
		EntityViewInfo ev2=new EntityViewInfo();
		FilterInfo filter2=new FilterInfo();
		filter2.getFilterItems().add(new FilterItemInfo("materialPacking.id","select FID from T_BD_Material where FNumber like '03%'",CompareType.INNER));
		ev2.setFilter(filter2);
		prmtmaterialPacking.setEntityViewInfo(ev2);*/
		/*EntityViewInfo ev1=new EntityViewInfo();
		FilterInfo filter1=new FilterInfo();
		filter1.getFilterItems().add(new FilterItemInfo("materialProduct.id","select FID from T_BD_Material where FNumber like '01%'",CompareType.INNER));
		ev1.setFilter(filter1);
