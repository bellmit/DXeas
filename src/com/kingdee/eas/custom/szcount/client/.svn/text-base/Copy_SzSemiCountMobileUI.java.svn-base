 ...
 * output class name
 */
public class Copy_SzSemiCountMobileUI extends AbstractCopy_SzSemiCountMobileUI
{
    private static final Logger logger = CoreUIObject.getLogger(Copy_SzSemiCountMobileUI.class);
    
    /**
     * output class constructor
     */
    public Copy_SzSemiCountMobileUI() throws Exception
    {
        super();		
		...
		//	if(countset.equals("0")){
				//将UI中各值添加在新增的成品计数上
					...
					for(String key :meterialdate.keySet()){
						...
						//关联成本对象
						String  costnumber = matet.getNumber();
						String  costnumberSQL ="select t1.FID from T_BD_CostObject t1 where t1.Fnumber ='"+costnumber+"' and t1.FControlUnitID = 'E3sAAAAAAPHM567U'";
						String  costID = null;
						IRowSet costRs = SQLExecutorFactory.getRemoteInstance(costnumberSQL).executeSQL();
						if(costRs.size()>0){
							while(costRs.next()){
								costID = costRs.getString("FID");
							}
							ICostObject iso = CostObjectFactory.getRemoteInstance();
...
//			}
	...
	private String createSeNumber() {
		...
		try {
			...
			SzSemiProductCollection coll;
			do{
 				str=MathUtilsComm.getStringByNumber(4, num);
				str = (new SimpleDateFormat("yyyyMMdd")).format(new Date())+str;
				if(countset.equals("0")){
					str = "BCPJS"+str;
					coll = SzSemiProductFactory.getRemoteInstance().getSzSemiProductCollection("where number='"+str+"'");
					if(coll.size()>0)
						return str;
					else
						num = num+coll.size();
...
//        this.classes.setRequired(false);
	...
	private void AllSql(String type,int minNum,int maxNum) throws BOSException, SQLException{
		String number = null;
		String personIDSql = SysContext.getSysContext().getCurrentUserInfo().getPerson().getId().toString();
		if(type.equals("xl")){
		...
		for(int i = 1;i<6;i++){
			...
			sql.append("left join CT_SZC_SzSemiProduct		 t10	on t10.CFPERsonID = '").append(personIDSql).append("'").append(sp);
			sql.append("left join CT_SZC_SzsemiProductEntry		 t9	on t4.FNumber = t9.CFMaterialNumber and t9.cfbatch = to_char(sysdate,'yyyyMMdd') and t10.FID = T9.FParentID").append(sp);
			sql.append("and t9.CFFinishedProductType ='"+typeString+"'").append(sp);
			sql.append(number).append(sp);
			sql.append("and tt1.CFCountSet = '").append(szcsInfo.getValue()).append("'").append(sp);
			sql.append("and tt3.CFPersonID = '").append(personIDSql).append("'").append(sp);
