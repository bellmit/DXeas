 ...
 * output class name
 */
public class freshBergamotCount extends AbstractfreshBergamotCount
{
    private static final Logger logger = CoreUIObject.getLogger(freshBergamotCount.class);
    
    /**
     * output class constructor
     */
    public freshBergamotCount() throws Exception
    {
        super();		
...
//        this.classes.setRequired(false);
	...
	private void TableNumber(String a){
		for(int i = 0;i<kDTable1.getRowCount();i++){
			if(kDTable1.getRow(i).getCell("materialNumber").getValue().equals(materialNumbers.getText())){
				if(measureType.getText().equals("盘")){
					if(kDTable1.getRow(i).getCell("disc").getValue()!=null){
						String kdNumber = (String) kDTable1.getRow(i).getCell("disc").getValue();
						StringBuffer weight = new StringBuffer();
						weight.append(kdNumber);
						weight.append(a);
						kDTable1.getRow(i).getCell("disc").setValue(weight.toString());
					}else{
						kDTable1.getRow(i).getCell("disc").setValue(a);
					}
				}
				if(measureType.getText().equals("包")){
					if(kDTable1.getRow(i).getCell("package").getValue()!=null){
						String kdNumber = (String) kDTable1.getRow(i).getCell("package").getValue();
						StringBuffer weight = new StringBuffer();
						weight.append(kdNumber);
						weight.append(a);
						kDTable1.getRow(i).getCell("package").setValue(weight.toString());
					}else{
						kDTable1.getRow(i).getCell("package").setValue(a);
					}
				}
				if(measureType.getText().equals("公斤")){
					if(kDTable1.getRow(i).getCell("item").getValue()!=null){
						String kdNumber = (String) kDTable1.getRow(i).getCell("item").getValue();
						StringBuffer weight = new StringBuffer();
						weight.append(kdNumber);
						weight.append(a);
						kDTable1.getRow(i).getCell("item").setValue(weight.toString());
					}else{
						kDTable1.getRow(i).getCell("item").setValue(a);
					}
				}
				break;
			}
