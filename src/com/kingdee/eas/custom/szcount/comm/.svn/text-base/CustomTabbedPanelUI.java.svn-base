package com.kingdee.eas.custom.szcount.comm;
...
public class CustomTabbedPanelUI extends BasicTabbedPaneUI {
	...
	public CustomTabbedPanelUI(String bgColorHex, String frontColorHex) {
		setColor(bgColorHex, frontColorHex);

		selectedColorSet = new ColorSet();
		selectedColorSet.topGradColor1 = Color.decode("#76EE00");// 选中的最上层
		selectedColorSet.topGradColor2 = Color.decode("#ADFF2F");// 选中的第二层
		selectedColorSet.bottomGradColor1 = Color.decode("#CAFF70");// 选中的第三层
	...
	protected int calculateTabHeight(int tabPlacement, int tabIndex, int fontHeight) {
		/*
		 * if (tabPlacement == LEFT || tabPlacement == RIGHT) { return
		 * super.calculateTabHeight(tabPlacement, tabIndex, fontHeight); } else
		 * { return anchoFocoH + super.calculateTabHeight(tabPlacement,
		 * tabIndex, fontHeight); }
		 */
		return 40;
	}
	...
	protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
		...
		int[] xp = new int[] { x, x, x + 3, x + w - inclTab - 6, x + w - inclTab - 3, x + w - inclTab, x + w - inclTab, x };
