...
//		String typeNumber = storeStateNumber;
	 ...
	 * @throws EASBizException 
	 */
	public PositionInfo getUserMainPosion(UserInfo userInfo) throws EASBizException, BOSException{
		if(userInfo != null){
			...
			if(personInfo != null){
				...
				sb.append(" top.fid ");
				sb.append(" from   ");
		//		sb.append(" inner join t_bd_person tp on t.cfpersonnumber = tp.fnumber ");
				sb.append("  T_HR_PersonPosition tpp ");
				sb.append(" inner join T_ORG_Position top on top.fid = tpp.fprimarypositionid ");
	//			sb.append(" inner join t_org_admin ta on top.fadminorgunitid = ta.fid ");
				sb.append("  where 1=1 ");
				
				sb.append(" and   tpp.fpersonid  = '");
				sb.append(personInfo.getId().toString());
				sb.append("' ");
				
				IRowSet rs = ToolFacadeFactory.getRemoteInstance().executeQuery(sb.toString());
				try{
					if(rs.next()){
						String fid = rs.getString(1);
						return PositionFactory.getRemoteInstance().getPositionInfo(new ObjectUuidPK(fid));
					}
