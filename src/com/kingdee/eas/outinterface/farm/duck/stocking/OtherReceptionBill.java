...
import com.kingdee.eas.farm.stocking.processbizill.IOtherReceptionBill;
 ...
 *
 */
public class OtherReceptionBill extends AbstractOutInterfaceImpl {
...
//		info.setSignData(null);
	...
	protected String[] getListSelector() {
		...
		return new String[]{"id","number","billStatus","farmer.name","bizDate",
				"stockingBatch.number","entrys.seq","entrys.materialName","entrys.unit.name","entrys.confirmQty"
				,"confirmTime"
		};
	}
	...
	public static String confirm(Context ctx,String jsonStr) {
		...
		try {
			IOtherReceptionBill is = OtherReceptionBillFactory.getLocalInstance(ctx);
			SelectorItemCollection slor=new SelectorItemCollection();
			
			OtherReceptionBillInfo info = is.getOtherReceptionBillInfo(new ObjectUuidPK(id));
...
//				info.setStorageOrgUnit(batchInfo.getStorageOrgUnit());
		...
		for(int index=0;index<info.getEntrys().size();index++) {
			OtherReceptionBillEntryInfo entryInfo = info.getEntrys().get(index);
...
//				entryInfo.setWarehouse(StockingComm.getDefaultWarehouseByMaterial(ctx, batchInfo.getStorageOrgUnit()!=null?batchInfo.getString("id"):"",  mInfo.getString("id")));
	...
	public void dealAfterSave(Context ctx, CoreBaseInfo model) throws BOSException, EASBizException {
		...
		OtherReceptionBillInfo info = (OtherReceptionBillInfo) model;
		OtherReceptionBillFactory.getLocalInstance(ctx).submit(info);
		/*if(info.getId()!=null) {
			//单据状态改为提交
			info.setBillStatus(BillBaseStatusEnum.SUBMITED);
			SelectorItemCollection slor=new SelectorItemCollection();
			slor.add("billStatus");
			OtherReceptionBillFactory.getLocalInstance(ctx).updatePartial(info, slor);
		}*/
	}
}