...
import com.kingdee.eas.bdm.bdapply.ICustomerInfoReq;
 ...
 * output class name
 */
public class CustomerInfoReqListUI extends AbstractCustomerInfoReqListUI
{
	private static final Logger logger = CoreUIObject.getLogger(CustomerInfoReqListUI.class);
	private static final Color APPROVEColor = new Color(252, 255, 206);
	/**
	 * output class constructor
	 */
	public CustomerInfoReqListUI() throws Exception
	{
		super();
	...
	public void actionAllAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		String number = "";
		CustomerInfoReqInfo info;
		ICustomerInfoReq is = CustomerInfoReqFactory.getRemoteInstance();
		for (int i = 0; i < ids.size(); ++i) {
			if (ids.get(i) == null)
				continue;
			info = is.getCustomerInfoReqInfo(new ObjectUuidPK(ids.get(i).toString()));
	...
	public void actionBasenotby_actionPerformed(ActionEvent e) throws Exception
	{
		...
		CustomerInfoReqInfo info = CustomerInfoReqFactory.getRemoteInstance().getCustomerInfoReqInfo(pk);
	...
	protected FilterInfo getDefaultFilterForQuery() {
		...
		try {
			...
			slor.add("*");
			CtrlUnitInfo ctrlUnitInfo = CtrlUnitFactory.getRemoteInstance().getCtrlUnitInfo(new ObjectUuidPK(cuID),slor );
			if(!cuID.equals("00000000-0000-0000-0000-000000000000CCE7AED4")){
				filter.getFilterItems().add(new FilterItemInfo("company.longNumber", ctrlUnitInfo.getLongNumber()+"%",CompareType.LIKE));
