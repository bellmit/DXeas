...
import com.kingdee.eas.weighbridge.IOtherStorageBill;
import com.kingdee.eas.weighbridge.IOtherStorageBillEntry;
import com.kingdee.eas.weighbridge.IOtherStorageBillEntryDetail;
import com.kingdee.eas.weighbridge.OtherStorageBillEntryCollection;
import com.kingdee.eas.weighbridge.OtherStorageBillEntryDetailCollection;
import com.kingdee.eas.weighbridge.OtherStorageBillEntryDetailFactory;
import com.kingdee.eas.weighbridge.OtherStorageBillEntryFactory;
import com.kingdee.eas.weighbridge.OtherStorageBillEntryInfo;
import com.kingdee.eas.weighbridge.OtherStorageBillFactory;
 ...
 * output class name
 */
public class GrossStorageBillPlatformUI extends AbstractGrossStorageBillPlatformUI
{
    private static final Logger logger = CoreUIObject.getLogger(GrossStorageBillPlatformUI.class);
    
    private OtherStorageBillInfo editData;// 过榜单基本数据
	public final static String billTable = "CT_WHB_OtherStorageBill";// 过榜单表
	public final static String billEntryTable = "CT_WHB_OtherStorageBillEntry";// 过榜单分录表
	public final static String billDetailTable = "CT_WHB_OtherSBED";// 过榜单明细表
...
//	private Boolean isNeedRead;
	 ...
	 */
	public GrossStorageBillPlatformUI() throws Exception {
		super();
		 ...
		 OrgUnitInfo orgInfo = SysContext.getSysContext().getCurrentOrgUnit();
		  ...
		  if(rs.next()) {
			  ...
			  LoadFields(null);
			  	...
			  	editData = new OtherStorageBillInfo();
...
//					if(this.cmbBizType.equals(WeighBizType.SaleReturn)){
//						orderMaps = weightbridgeFacadeFactory.getRemoteInstance().getPurReturnsOrderInfoByNum(orderNum,carnumber,beginDateStr,endDateStr);
//					}else{
...
//				Date beginDate = cal.getTime();
	 ...
	 * @return
	 */
	private OtherStorageBillInfo getNewWeightInBill(OrderWeightInfo orderInfo){
		OtherStorageBillInfo inStorageBillInfo = new OtherStorageBillInfo();
...
//	}
	public void actionCreateFrom_actionPerformed(ActionEvent e) throws Exception {
		...
		try{
			...
			if(destBillCols!=null&&destBillCols.size()>0) {
				String entryID=null;
				OtherStorageBillInfo head = (OtherStorageBillInfo) destBillCols.getObject(0);
	 ...
	 * @throws BOSException
	 */
	private IOtherStorageBill getBizInstance() throws BOSException {
		return OtherStorageBillFactory.getRemoteInstance();
	}

	private IOtherStorageBillEntry getBizEntryInstance() throws BOSException {
		return OtherStorageBillEntryFactory.getRemoteInstance();
	}

	private IOtherStorageBillEntryDetail getBizEntryDetailInstance() throws BOSException {
		return OtherStorageBillEntryDetailFactory.getRemoteInstance();
	}
...
//			this.dpOutTime.setValue(editData.getOutTime());
			...
			//遍历分录集合、累加净重合计
			OtherStorageBillEntryCollection entryCol = this.editData.getEntrys();
...
//						this.txtModel.setText(entryCol.get(i).getMaterial().getModel());
	...
	private void setNewBillInfo(String billID) throws EASBizException, BOSException {
		...
		if (StringUtils.isBlank(billID)) {
			...
			editData = this.getBizInstance().getOtherStorageBillInfo(new ObjectUuidPK(billID), slors);
		}
	}
...
//		editData.setOutTime((Date) this.dpOutTime.getValue());
		...
		if (this.editData.getEntrys().size() <= 0) {
			OtherStorageBillEntryInfo entryInfo = new OtherStorageBillEntryInfo();
...
//				}
				...
				OtherStorageBillEntryDetailCollection detailCol = this.editData.getEntrys().get(i).getDetail();
	 ...
	 * @return
	 */
	private BigDecimal getDetailQty(OtherStorageBillEntryInfo entryInfo) {
...
//		}
	...
	private void checkOrderQty() throws BOSException, SQLException {
		...
		for (int j = 0; j < this.editData.getEntrys().size(); j++) {
			OtherStorageBillEntryDetailCollection detailCol = this.editData.getEntrys().get(j).getDetail();
...
//			row.getCell("model").setValue(rs.getString("FModel"));
	...
	private IRowSet getExeQueryRowSet(OrgUnitInfo orgInfo,String beginDate,String endDate,String materialID,String carID,String otherCondition,String carNumber) throws BOSException {
		StringBuffer sql = new StringBuffer();
		sql.append(" select tb.cfpurordernumber orderNumber,tb.FID,te.FID FEntryID,tm.FID FMaterialID,tm.FNumber FMaterialNum,tm.FName_l2 FMaterialName,tm.FModel,");
...
//			sql.append(" and tb.CFCarID='").append(carID).append("'");;
	...
	protected void disposeUIWindow() {
		isNeedExitList.put(this.getClass().getSimpleName(), true);
		super.disposeUIWindow();
		isNeedExitList.remove(this.getClass().getSimpleName());
	}
