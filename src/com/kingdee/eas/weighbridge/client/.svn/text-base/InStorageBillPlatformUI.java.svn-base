...
import com.kingdee.eas.custom.dx.weight.CarSendBill;
 ...
 * output class name
 */
public class InStorageBillPlatformUI extends AbstractInStorageBillPlatformUI {
	private static final Logger logger = CoreUIObject.getLogger(InStorageBillPlatformUI.class);
	private InStorageBillInfo editData;// 过榜单基本数据
	public final static String billTable = "CT_WHB_InStorageBill";// 过榜单表
	public final static String billEntryTable = "CT_WHB_InStorageBillEntry";// 过榜单分录表
	 ...
	 */
	public InStorageBillPlatformUI() throws Exception {
		super();
...
//			  LoadFields(null);
//		  }else {
//			  	editData = new InStorageBillInfo();
...
//				MsgBox.showWarning("请至少输入派车单号或者车牌号至少一个值进行查询");
//				return;
//			}else{
				try{
					...
					Map<String,OrderWeightInfo> orderMaps = weightbridgeFacadeFactory.getRemoteInstance().getPurorderInfoByNumber(orderNum,carnumber,beginDateStr,endDateStr);
...
//				Date beginDate = cal.getTime();
	 ...
	 * @return
	 */
	private InStorageBillInfo getNewWeightInBill(OrderWeightInfo orderInfo){
		
		String fid = CheckBill(orderInfo);
		if(StringUtils.isNotBlank(fid)){
			InStorageBillInfo inStorageBillInfo;
			try {
				inStorageBillInfo = InStorageBillFactory.getRemoteInstance().getInStorageBillInfo(new ObjectUuidPK(fid));
			...
			inStorageBillInfo = new InStorageBillInfo();
			...
			InStorageBillInfo inStorageBillInfo = new InStorageBillInfo();
			inStorageBillInfo.setId(BOSUuid.create(inStorageBillInfo.getBOSType().toString()));
			inStorageBillInfo.setNumber(getAutoNumberByOrg("NONE"));
			inStorageBillInfo.setCompany(SysContext.getSysContext().getCurrentFIUnit());
			inStorageBillInfo.setId(BOSUuid.create(inStorageBillInfo.getBOSType().toString()));
...
//		}
	...
	private String CheckBill(OrderWeightInfo orderInfo) {
		...
		str.append("select FID from CT_WHB_InStorageBill ");
	...
	public void actionCreateFrom_actionPerformed(ActionEvent e) throws Exception {
		...
		try{
			...
			if(destBillCols!=null&&destBillCols.size()>0) {
				String entryID=null;
				InStorageBillInfo head = (InStorageBillInfo) destBillCols.getObject(0);
	 ...
	 * @throws BOSException
	 */
	private IInStorageBill getBizInstance() throws BOSException {
		return InStorageBillFactory.getRemoteInstance();
	}

	private IInStorageBillEntry getBizEntryInstance() throws BOSException {
		return InStorageBillEntryFactory.getRemoteInstance();
	}

	private IInStorageBillEntryDetail getBizEntryDetailInstance() throws BOSException {
		return InStorageBillEntryDetailFactory.getRemoteInstance();
	}
	...
	protected void btnSave_actionPerformed(ActionEvent e) throws Exception {
		...
		try {
			...
			IInStorageBill  pki=InStorageBillFactory.getRemoteInstance();
...
//			}
			...
			//遍历分录集合、累加净重合计
			InStorageBillEntryCollection entryCol = this.editData.getEntrys();
...
//					}
	...
	private void setNewBillInfo(String billID) throws EASBizException, BOSException {
		...
		if (StringUtils.isBlank(billID)) {
			...
			editData = this.getBizInstance().getInStorageBillInfo(new ObjectUuidPK(billID), slors);
		}
	}
...
//		}
		...
		if (this.editData.getEntrys().size() <= 0) {
			InStorageBillEntryInfo entryInfo = new InStorageBillEntryInfo();
...
//				InStorageBillEntryDetailCollection detailCol = this.editData.getEntrys().get(i).getDetail();
...
//	}
	...
	private void execQuery() throws BOSException, SQLException {
		...
		if(txtOrderNum.getText()!=null){
			...
			while (rs.next()) {
				...
				if(rs.getString("CFOutTime")!=null) {
	...
	private IRowSet getExeQueryRowSet(String carID) throws BOSException {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" select tb.cfpurordernumber orderNumber,tb.FID,te.FID FEntryID,te.FSeq,te.CFWeighTimeGross CFOutTime,");
		sql.append(" tb.FBizDate,te.CFGross,tb.FNumber,");
