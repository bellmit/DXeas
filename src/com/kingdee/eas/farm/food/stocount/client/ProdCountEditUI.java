 ...
 * output class name
 */
public class ProdCountEditUI extends AbstractProdCountEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProdCountEditUI.class);
    
    /**
     * output class constructor
     */
    public ProdCountEditUI() throws Exception
    {
        super();
    }
    /**
	...
	/* (non-Javadoc)
	 * @see com.kingdee.eas.farm.food.stocount.client.AbstractProdCountEditUI#onShow()
	 */
	@Override
    ...
    {
    	//editData.setPerson(SysContext.getSysContext().getCurrentUserInfo().getPerson() );
    	
    	int row = this.kdtEntrys.getRowCount();
    	if(row>1){
    		MsgBox.showWarning("当前分录记录行不能多于一行！");
			SysUtil.abort();
    	}
        super.actionSave_actionPerformed(e);
        String bzid = editData.getDescription().toString(); 
        String sql = "update CT_FM_PcakCount set CFIsPackAcceptance = '1' where  FNumber = '"+bzid+"'";
    ...
    {
    	if(FarmCommUtils.checkBillStatus(this.editData.getStatus(),BillBaseStatusEnum.AUDITED)) {
    ...
    public void actionRemove_actionPerformed(ActionEvent e) throws Exception
    {
    	if(FarmCommUtils.checkBillStatus(this.editData.getStatus(),BillBaseStatusEnum.AUDITED)) {
    	 ...
    	 String bzid = editData.getDescription().toString(); 
    	 String sql = "update CT_FM_PcakCount set CFIsPackAcceptance = '0' where  FNumber = '"+bzid+"'";
    	 CommFacadeFactory.getRemoteInstance().excuteUpdateSql(sql);
    ...
    public void actionAudit_actionPerformed(ActionEvent e) throws Exception
    {
    	...
    	if(!FarmCommUtils.checkBillStatus(this.editData.getStatus(),BillBaseStatusEnum.SUBMITED)) {
    		MsgBox.showWarning("单据尚未提交，禁止审核！");
    		SysUtil.abort();
    	}
		super.actionAudit_actionPerformed(e);
		this.editData.setStatus(BillBaseStatusEnum.AUDITED);
    ...
    protected com.kingdee.bos.dao.IObjectValue createNewData()
    {
      ...
      //user和administrator没有员工内码所以不能进行岗位判断
		...
		objectValue.setPerson(SysContext.getSysContext().getCurrentUserInfo().getPerson() );
//		this.prmtcontperson.setValue(SysContext.getSysContext().getCurrentUserInfo().getPerson() );s
		//this.contcontperson.set
		...
		try {
			...
			if(cpso.size()==0){
				MsgBox.showWarning("当前用户没有计数岗位权限，无法新增计数单！");
				SysUtil.abort();
			}
