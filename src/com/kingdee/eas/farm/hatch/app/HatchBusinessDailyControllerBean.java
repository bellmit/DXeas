...
import com.kingdee.eas.farm.hatch.HatchBusinessDailyFactory;
...
public class HatchBusinessDailyControllerBean extends AbstractHatchBusinessDailyControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.farm.hatch.app.HatchBusinessDailyControllerBean");
	...
	protected IObjectPK _save(Context ctx, IObjectValue model)
		...
		// 状态设置为保存
		HatchBusinessDailyInfo info =  (HatchBusinessDailyInfo)model;
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException,
		...
		HatchBusinessDailyInfo info =  HatchBusinessDailyFactory.getLocalInstance(ctx).getHatchBusinessDailyInfo(pk);
		if(!BillBaseStatusEnum.TEMPORARILYSAVED.equals(info.getBaseStatus()) &&
				!BillBaseStatusEnum.ADD.equals(info.getBaseStatus())	&& 
	...
	protected IObjectPK _submit(Context ctx, IObjectValue model)
			throws BOSException, EASBizException {
		HatchBusinessDailyInfo billInfo = (HatchBusinessDailyInfo)model;
    	if(billInfo != null  && 
	...
	protected void _audit(Context ctx, IObjectValue model) throws BOSException {
		HatchBusinessDailyInfo billInfo = (HatchBusinessDailyInfo)model;
			...
			if(billInfo != null && billInfo.getBaseStatus() != null && BillBaseStatusEnum.SUBMITED.equals(billInfo.getBaseStatus())){
				...
				// 更新单据状态
				AppCommon.updateModelStatus(ctx, "CT_FM_HatchBusinessDaily", billInfo.getId().toString(), BillBaseStatusEnum.AUDITED);
	...
	protected void _unAudit(Context ctx, IObjectValue model)
		...
		if(billInfo != null && billInfo.getBaseStatus() != null && BillBaseStatusEnum.AUDITED.equals(billInfo.getBaseStatus())){
			...
			// 更新单据状态
			AppCommon.updateModelStatus(ctx, "CT_FM_HatchBusinessDaily", billInfo.getId().toString(), BillBaseStatusEnum.TEMPORARILYSAVED);
