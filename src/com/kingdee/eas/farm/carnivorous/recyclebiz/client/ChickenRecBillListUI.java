 ...
 * output class name
 */
public class ChickenRecBillListUI extends AbstractChickenRecBillListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ChickenRecBillListUI.class);
    
    /**
     * output class constructor
     */
    public ChickenRecBillListUI() throws Exception
    {
        super();
	...
	{	
		...
		IChickenRecBill iBill = ChickenRecBillFactory.getRemoteInstance();
		ChickenRecBillInfo info;
		int num=selIDs.length;
		String error="";
		for (int i = 0;i<selIDs.length; i++) {
			info = iBill.getChickenRecBillInfo(new ObjectUuidPK(selIDs[i]));
	...
	public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception
	{
		...
		try{
			if(num==selIDs.length&&num==1)
				...
				MsgBox.showInfo("反审核失败,没有符合反审核条件的单子,并检查是否关账！");
			else{
    ...
    @Override
    public void actionManageCarSeq_actionPerformed(ActionEvent e)
    		throws Exception {
		...
		ChickenRecBillInfo info=ChickenRecBillFactory.getRemoteInstance().getChickenRecBillInfo(new ObjectUuidPK(id),slor);
		if(info.getBillStatus()!=BillBaseStatusEnum.AUDITED){
			MsgBox.showWarning("审核状态的单据才能执行此操作!");
			return;
		}

//		if(info.isIsHasPaiChe()){
//			MsgBox.showWarning("派过车的订单不能执行此操作!");
//			return;
//		}
//		if(info.getSaleType()==SaleBizType.out){
//			MsgBox.showWarning("外销毛鸡无需派车!");
//			return;
//		}
		...
		try {
			...
			IUIWindow groupDialog = uiFactory.create("com.kingdee.eas.farm.carnivorous.recyclebiz.client.CarSeqManageUI", map);
			groupDialog.show();
