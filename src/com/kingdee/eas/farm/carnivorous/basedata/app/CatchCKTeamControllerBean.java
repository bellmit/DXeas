...
import com.kingdee.eas.farm.carnivorous.basedata.CatchCKTeamFactory;
...
public class CatchCKTeamControllerBean extends AbstractCatchCKTeamControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.farm.carnivorous.basedata.app.CatchCKTeamControllerBean");
    
    protected IObjectPK _save(Context ctx, IObjectValue model)
	throws BOSException, EASBizException {
		CatchCKTeamInfo info = (CatchCKTeamInfo ) model;		
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		// TODO Auto-generated method stub
		CatchCKTeamInfo info = CatchCKTeamFactory.getLocalInstance(ctx).getCatchCKTeamInfo(pk);
		...
		sqlBuf.append(" select fid from CT_FM_CSlaughterBill where CFCatchCKTeamID='"+info.getId().toString()+"' ");
		IRowSet rs = SQLExecutorFactory.getLocalInstance(ctx, sqlBuf.toString()).executeSQL();
		if(rs.size()>0){
			throw new EASBizException(new NumericExceptionSubItem("001","该基础资料已被其他单据引用,不能删除！"));
		}
