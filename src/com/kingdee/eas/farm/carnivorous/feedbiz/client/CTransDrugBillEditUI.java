...
import com.kingdee.eas.farm.carnivorous.feedbiz.CTransDrugBillEntry;
 ...
 * output class name
 */
public class CTransDrugBillEditUI extends AbstractCTransDrugBillEditUI
{
	private static final Logger logger = CoreUIObject.getLogger(CTransDrugBillEditUI.class);
	 ...
	 */
	public CTransDrugBillEditUI() throws Exception{
		super();
	...
	public void loadFields(){
		...
		UITools.apendFootRow(kdtEntrys, new String[]{"transQty","orderAmount","amount"});
	...
	protected void verifyInput(ActionEvent e) throws Exception {
		...
		for(int rowIndex=0;rowIndex<this.kdtEntrys.getRowCount();rowIndex++) {
			...
			if(transQty.signum()<=0){
				MsgBox.showWarning("分录第"+(rowIndex+1)+"行转药数量不能为空或小于等于0");
				SysUtil.abort();
			}
	...
	private void initControl() throws Exception {
		...
		kdtEntrys_detailPanel.addAddListener(new DetailPanelBackListener(){
			public void beforeEvent(DetailPanelEvent e) throws Exception {
				// TODO Auto-generated method stub
				super.beforeEvent(e);
				CTransDrugBillEntryInfo entry=(CTransDrugBillEntryInfo)e.getObjectValue();
				entry.setWarehouse(wareHouseInfo);		
				entry.setOrderWarehouse(wareHouseInfo);	
			}
		});
	...
	public void kdtEntrys_Changed(int rowIndex, int colIndex) throws Exception {
		...
		if("material".equals(kdtEntrys.getColumnKey(colIndex))){
			if(kdtEntrys.getCell(rowIndex, "material").getValue()!=null){
				...
				kdtEntrys.getCell(rowIndex, "unit").setValue(null);
			}
		}
		calAmount(rowIndex);
