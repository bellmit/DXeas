...
import com.kingdee.eas.custom.commld.fileUtils.Picture.PictureViewer;
 ...
 * output class name
 */
public class FarmPollingBillEditUI extends AbstractFarmPollingBillEditUI
{
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final Logger logger = CoreUIObject.getLogger(FarmPollingBillEditUI.class);
	ArrayList<File> picturePath = new ArrayList<File>();
	String picPath = "";
    /**
     * output class constructor
     */
    public FarmPollingBillEditUI() throws Exception
    {
        super();
    }
    @Override
		...
		prmtbatch.addDataChangeListener(new DataChangeListener(){
			public void dataChanged(DataChangeEvent arg0) {
				if(prmtbatch.getValue()!=null){
					try {
						String batchID=((IObjectValue)prmtbatch.getValue()).getString("id");
						...
						prmtbatchContract.setValue(batchInfo.getBatchContract());
					} catch (Exception e) {
						e.printStackTrace();
					}
    ...
    @Override
	public void actionDisplayPic_actionPerformed(ActionEvent e)
	throws Exception {
		// TODO Auto-generated method stub
		super.actionDisplayPic_actionPerformed(e);
		if(editData.getId() != null)
			try {
				...
				for(int i = 0;i<array.size();i++){
					...
					if(StringUtils.isNotEmpty(simpleName)&& commUtils.arrayListContainsElement(simpleName, new String[]{"png","gif","jpeg","bmp","jpg"})){
						String folder = System.getProperty("java.io.tmpdir").replace("ADMINI~1", "Administrator");
						String filePath = folder+"EAS";
						File file = null;
						file = new File(filePath);
						if(!file.exists() || !file.isDirectory()){
							file.mkdir();
						}
						filePath = filePath + "\\"+fileName+"."+simpleName;
						file = new File(filePath);
						...
						file.createNewFile();
						byte[] buffer = decoder.decodeBuffer(fileContent);
			...
			if(picturePath.size()==0){
				commUtils.giveUserTipsAndRetire("没有找到附件中上传的图片");
			}
			pictureViewer.setFiles(picturePath.toArray(new File[picturePath.size()]));
			pictureViewer.setFilesIndex(0);
			pictureViewer.init();
			pictureViewer.setSelectButtonVisible(false);
			boolean display = pictureViewer.display(pictureViewer.getFiles()[0]);
//			String path = pictureViewer.getFiles()[0].getPath();
//			if(!display){
