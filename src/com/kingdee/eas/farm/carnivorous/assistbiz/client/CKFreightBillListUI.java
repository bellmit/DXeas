	...
	throws Exception {	
		...
		for (int i = 0;i<selIDs.length; i++) {
			...
			if(info.getDistance()==null){
				error+=info.getString("number")+"\t"+"未执行日运费计算"+"\n";
				num--;
				continue;
			}
			try{
	...
	throws Exception {
		...
		for (int i = 0;i<selIDs.length; i++) {
			...
			if(info.getCalUnit()==null){
				error+=info.getString("number")+"\t"+"运费计算点不能为空"+"\n";
				num--;
				continue;
			}
			try{
		...
		try{
			if(num==selIDs.length&&num==1)
				...
				MsgBox.showInfo("计算成功(操作单子数:"+selIDs.length+")");
			else if(num==0)
				throw new EASBizException(new NumericExceptionSubItem("","计算失败!\n"+error));
			else{
				throw new EASBizException(new NumericExceptionSubItem("","操作完成但存在异常，共有"+num+"张单子计算完成\n"+error));
			}
		} 
		finally{
			refreshList();
