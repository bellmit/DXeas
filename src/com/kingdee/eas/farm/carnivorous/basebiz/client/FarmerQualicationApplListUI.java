 ...
 * output class name
 */
public class FarmerQualicationApplListUI extends AbstractFarmerQualicationApplListUI
{
	private static final Logger logger = CoreUIObject.getLogger(FarmerQualicationApplListUI.class);

	/**
	 * output class constructor
	 */
	public FarmerQualicationApplListUI() throws Exception
	{
		super();
	}
	...
	private void selectedRowChanged(int rowIndex) {
		...
		if(list.size()>0) {
			...
			try {
				if(this.getBizInterface().getValue(new ObjectUuidPK(getSelectedKeyValue())).get("farmer")!=null) {
					this.actionUnAudit.setEnabled(false);
				}
			} catch (Exception e) {
