 ...
 * output class name
 */
public class DrugReceptionBillListUI extends AbstractDrugReceptionBillListUI
{
	private static final Logger logger = CoreUIObject.getLogger(DrugReceptionBillListUI.class);

	/**
	 * output class constructor
	 */
	public DrugReceptionBillListUI() throws Exception
	{
		super();
	}
...
//	}
	...
	public void actionCreateTo_actionPerformed(ActionEvent e) throws Exception {
		...
		if(destObejctIDs.size()>0)
			CommFacadeFactory.getRemoteInstance().excuteUpdateSql("update T_FM_DrugReceptionBill set FIsHasMaterial=1 where fid='"+pk+"'");
		
		//校验生成库存调拨单
		destObejctIDs = BOTRelationFactory.getRemoteInstance().getDestObjIdByDestType(pk, "2239F30A");
		if(destObejctIDs.size()>0)
			CommFacadeFactory.getRemoteInstance().excuteUpdateSql("update T_FM_DrugReceptionBill set FIsHasTrans=1 where fid='"+pk+"'");
		refreshList();
	}
