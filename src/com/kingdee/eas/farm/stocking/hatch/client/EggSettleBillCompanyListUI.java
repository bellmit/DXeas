 ...
 * output class name
 */
public class EggSettleBillCompanyListUI extends AbstractEggSettleBillCompanyListUI
{
    private static final Logger logger = CoreUIObject.getLogger(EggSettleBillCompanyListUI.class);
    
    /**
     * output class constructor
     */
    public EggSettleBillCompanyListUI() throws Exception
    {
        super();
    }
	...
	protected FilterInfo getDefaultFilterForQuery() {
		...
		if(SysContext.getSysContext().getCurrentOrgUnit().isIsCompanyOrgUnit()) {
			...
			if(SysContext.getSysContext().getCurrentStorageUnit()!=null) {
				orgId=SysContext.getSysContext().getCurrentStorageUnit().getString("id");
			}
			filter.getFilterItems().add(new FilterItemInfo("storageOrgUnit.id",orgId,CompareType.EQUALS));
		}
		return filter;
	}
