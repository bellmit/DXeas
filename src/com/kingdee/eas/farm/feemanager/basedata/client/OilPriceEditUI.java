 ...
 * output class name
 */
public class OilPriceEditUI extends AbstractOilPriceEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(OilPriceEditUI.class);
  
    /**
     * output class constructor
     */
    public OilPriceEditUI() throws Exception
    {
        super();
      //图标初始化
    ...
    public void loadFields()
    {
        ...
        if((BillBaseStatusEnum.AUDITED).equals(this.state.getSelectedItem())){
        ...
        } else if((BillBaseStatusEnum.SUBMITED).equals(this.state.getSelectedItem())){
        ...
        }else if((BillBaseStatusEnum.ADD).equals(this.state.getSelectedItem())){
        	this.actionReview.setEnabled(true);
        	this.actionAddNew.setEnabled(true);
        	this.actionEdit.setEnabled(true);
        	this.unreview.setEnabled(false);
        	...
        	this.actionSave.setEnabled(true);
        	this.actionSubmit.setEnabled(true);
        }else if((BillBaseStatusEnum.TEMPORARILYSAVED).equals(this.state.getSelectedItem())){
        	this.actionReview.setEnabled(true);
        	...
        	this.actionSave.setEnabled(true);
        	this.actionSubmit.setEnabled(true);
        }else if((BillBaseStatusEnum.DELETED).equals(this.state.getSelectedItem())){
        	this.actionReview.setEnabled(false);
        	this.actionAddNew.setEnabled(false);
        	this.actionEdit.setEnabled(false);
        	this.unreview.setEnabled(false);
	...
	public void actionReview_actionPerformed(ActionEvent e) throws Exception {
		...
		if(FarmCommUtils.checkBillStatus(this.editData.getState(),BillBaseStatusEnum.AUDITED)) {
    		MsgBox.showWarning("单据已经核审，禁止再次核审！");
    		SysUtil.abort();
    	}
    	if(!FarmCommUtils.checkBillStatus(this.editData.getState(),BillBaseStatusEnum.SUBMITED)) {
    		MsgBox.showWarning("单据尚未提交，禁止核审！");
    		SysUtil.abort();
    	}
	...
	public void actionUnreview_actionPerformed(ActionEvent e) throws Exception {
		...
		if(!FarmCommUtils.checkBillStatus(this.editData.getState(),BillBaseStatusEnum.AUDITED)) {
    		MsgBox.showWarning("单据尚未核审，禁止反核审！");
    		SysUtil.abort();
    	}
    ...
    public void actionSave_actionPerformed(ActionEvent e) throws Exception
    {
		...
		try {
			...
			.getRemoteInstance().oiljudg(str,this.editData.getStartdate().toString());
			IRowSet rs = (IRowSet) result.get("result");
			if(rs.size()!=0){
				while (rs.next()) {
					MsgBox.showWarning("重复定义同一日期的柴油价格！");
					SysUtil.abort();
				}
			}		
    ...
    public void setDataObject(IObjectValue dataObject) 
    {
        ...
        if(STATUS_ADDNEW.equals(getOprtState())) {
            editData.put("treeid",(com.kingdee.eas.farm.feemanager.basedata.OilPriceTreeInfo)getUIContext().get(UIContext.PARENTNODE));
        }
    }
    ...
    protected com.kingdee.bos.dao.IObjectValue createNewData()
    {
    	...
    	CompanyOrgUnitInfo companyOrgUnitinfo = (CompanyOrgUnitInfo)prmtcompanyorgunit.getValue();
    	objectValue.setStartdate(new Date());
    	//String str = "where fCompanyOrgUnitid = '"+companyOrgUnitinfo.getId().toString()+"'";
    	//String str = "where companyorgunit = '"+SysContext.getSysContext().getCurrentFIUnit().getId().toString()+"'";
    	//System.out.println("财务组织ID："+SysContext.getSysContext().getCurrentFIUnit().getId().toString());
    	
    	
    	//String str = this.prmtcompanyorgunit.getValue().toString();
