...
/**
 * 获取饲料调拨计划 料塔明细   计划直接获取 要料数据
 * 
 * 获取 要料计划、理论需求量、实际调拨量 三组数据
 * @author USER
 *
 */
public class CCTowerFodderDetailPlanRptFacadeControllerBean extends AbstractCCTowerFodderDetailPlanRptFacadeControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.farm.rpt.app.CCTowerFodderDetailPlanRptFacadeControllerBean");
...
//			System.out.println(new Date());
			...
			if("1".equals(sumType)){
				...
				sumsql.append("          group by tmmmm.stoOrgName,  tmmmm.currentDate	");
				sumsql.append("          order by tmmmm.stoOrgName, tmmmm.currentDate ");
				...
				sumsql.append(temptableName);
				sumsql.append("' temptableName,");
				sumsql.append("   max(tmmm.OneMaterialPlanQty) OneMaterialPlanQty,	");
				sumsql.append("   max(tmmm.OneMaterialPredictQty) OneMaterialPredictQty,	");
				sumsql.append("   max(tmmm.OneMaterialActualQty) OneMaterialActualQty,	");
				
				sumsql.append("   max(tmmm.TwoMaterialPlanQty) TwoMaterialPlanQty,	");
				sumsql.append("   max(tmmm.TwoMaterialPredictQty) TwoMaterialPredictQty,	");
				sumsql.append("   max(tmmm.TwoMaterialActualQty) TwoMaterialActualQty,	");
				
				sumsql.append("   sum(decode(tdfpme3.fid,null,0,tdfpe3.CFFODDERQTY))/1000 ThreeMaterialPlanQty,	");
				sumsql.append("   max(tmmm.ThreeMaterialPredictQty) ThreeMaterialPredictQty,	");
				sumsql.append("   sum(decode(tsbem3.fid,null,0,tsbe3.fbaseqty))/1000 ThreeMaterialActualQty	");
//				sumsql.append("   sum(tsbe3.fbaseqty）/1000 ThreeMaterialActualQty	");
				...
				sumsql.append("   max(tmm.OneMaterialPlanQty) OneMaterialPlanQty,	");
				sumsql.append("   max(tmm.OneMaterialPredictQty) OneMaterialPredictQty,	");
				sumsql.append("   max(tmm.OneMaterialActualQty) OneMaterialActualQty,	");
				
				sumsql.append("   sum(decode(tdfpme2.fid,null,0,tdfpe2.CFFODDERQTY))/1000 TwoMaterialPlanQty,	");
				sumsql.append("   max(tmm.TwoMaterialPredictQty) TwoMaterialPredictQty,	");
				sumsql.append("   sum(decode(tsbem2.fid,null,0,tsbe2.fbaseqty))/1000 TwoMaterialActualQty,	");
//				sumsql.append("   sum(tsbe2.fbaseqty）/1000 TwoMaterialActualQty,	");
				...
				sumsql.append(" from (	");
				sumsql.append(" select max(tm.farmID) farmID,tm.stoOrgName, tm.batchName, tm.towerName, tm.currentDate, max(tm.towerid) towerID,	");
				sumsql.append("   sum(decode(tdfpme1.fid,null,0,tdfpe1.CFFODDERQTY))/1000 OneMaterialPlanQty,	");
				sumsql.append("   max(tm.OneMaterialPredictQty) OneMaterialPredictQty,	");
				sumsql.append("   sum(decode(tsbem1.fid,null,0,tsbe1.fbaseqty))/1000 OneMaterialActualQty,	");
//				sumsql.append("   sum(tsbe1.fbaseqty) /1000 OneMaterialActualQty,	");
				...
				sumsql.append("          left join T_BD_Material tsbem1 on tsbe1.fmaterialid  = tsbem1.fid and tsbem1.fname_l2 like '%RJ-01%' ");
				// 日报要料计划一号料
				sumsql.append("          left join ct_fm_commecialcdfpe tdfpe1 on  to_char(tdfpe1.CFFODDERDATE,'yyyy-mm-dd') = to_char(tm.currentDate,'yyyy-mm-dd') and tm.towerID = tdfpe1.CFFODDERTOWERID ");
				sumsql.append("          left join t_bd_material tdfpme1 on  tdfpe1.CFFODDERMATERIALID = tdfpme1.fid and tdfpme1.fname_l2 like  '%RJ-01%' ");
				...
				// 日报要料计划二号料
				sumsql.append("          left join ct_fm_commecialcdfpe tdfpe2 on  to_char(tdfpe2.CFFODDERDATE,'yyyy-mm-dd') = to_char(tmm.currentDate,'yyyy-mm-dd') and tmm.towerID = tdfpe2.CFFODDERTOWERID ");
				sumsql.append("          left join t_bd_material tdfpme2 on  tdfpe2.CFFODDERMATERIALID = tdfpme2.fid and tdfpme2.fname_l2 like  '%RJ-02%' ");
				...
				// 日报要料计划三号料
				sumsql.append("          left join ct_fm_commecialcdfpe tdfpe3 on  to_char(tdfpe3.CFFODDERDATE,'yyyy-mm-dd') = to_char(tmmm.currentDate,'yyyy-mm-dd') and tmmm.towerID = tdfpe3.CFFODDERTOWERID ");
				sumsql.append("          left join t_bd_material tdfpme3 on  tdfpe3.CFFODDERMATERIALID = tdfpme3.fid and tdfpme3.fname_l2 like  '%RJ-03%' ");
				
				
				sumsql.append("          group by tmmm.stoOrgName, tmmm.batchName, tmmm.towerName, tmmm.currentDate	");
				sumsql.append("          order by tmmm.stoOrgName, tmmm.batchName, tmmm.towerName, tmmm.currentDate	");
				sumsql.append("          	");
