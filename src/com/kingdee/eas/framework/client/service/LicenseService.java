...
public class LicenseService {
	...
	public void checkLicence() throws Exception {
		LicenseUserInfo user = (LicenseUserInfo) SysContext.getSysContext()
				.getProperty("License.UserInfo");
		...
		case 1:
			...
			break;
		case 4:
			MsgBox.showDetailAndOK(
							(CoreUI) this.ui,
							EASResource
									.getString("com.kingdee.eas.framework.FrameWorkResource.Error_License_Invalid"),
							constructMessage(lc, className), 0);
		...
		case 2:
			MsgBox
					.showDetailAndOK(
							(CoreUI) this.ui,
							EASResource
									.getString("com.kingdee.eas.framework.FrameWorkResource.Error_License_NO_SURPLUS"),
							constructMessage(lc, className), 0);
	...
	public void releaseLicense() throws Exception {
		if (this.hasLicence) {
			...
			try {
				...
				String moduleName = (String) this.ui.getUIContext().get(
						MODULENAME);
				if (!(StringUtils.isEmpty(moduleName))) {
	...
	public void userDefReleaseLicense(LicenseController lc, String className,
			String moduleName) throws Exception {
		if (this.hasLicence) {
			String sessionId = SysContext.getSysContext().getSessionID();
			try {
				lc.releaseLicenseByModuleAndUIName(sessionId, moduleName,
						className);
	...
	public void userDefRequestLicense(LicenseController lc, String className,
			String moduleName) throws Exception {
		LicenseUserInfo user = (LicenseUserInfo) SysContext.getSysContext()
				.getProperty("License.UserInfo");

		int licenselcFlag = lc.reqeustLicenseByModuleAndUIName(user,
		...
		case 4:
			MsgBox
					.showDetailAndOK(
							...
							EASResource
									.getString("com.kingdee.eas.framework.FrameWorkResource.Error_License_Invalid"),
							constructMessageByModuleName(lc, moduleName), 0);
	...
	private String constructMessageByModuleName(LicenseController lc,
			String moduleName) throws LicenseException {
		...
		buffer
				.append(EASResource
						.getString("com.kingdee.eas.framework.FrameWorkResource.Msg_ClassName"));
...
//				&& (check.trim().equalsIgnoreCase("true"))) {
...
//		SysUtil.abort();
	...
	public static final String constructLicenseMessage(LicenseController lc,
		...
		buffer
				.append(EASResource
						.getString("com.kingdee.eas.framework.FrameWorkResource.Msg_ModuleName"));
		String moduleName = lc.getModuleByPackage(className);
		if ((moduleName == null) || (moduleName.trim().length() == 0))
			buffer
					.append(EASResource
							.getString("com.kingdee.eas.framework.FrameWorkResource.Msg_UnlimitUI"));
		else {
			buffer.append(LicenseClientUtil
					.getModularAliasNameByModularName(moduleName)
					+ "[" + moduleName + "]");
		}
		buffer.append("\n");
		buffer
				.append(EASResource
						.getString("com.kingdee.eas.framework.FrameWorkResource.Msg_SubSystem"));
		String subSysName = LicenseClientUtil
				.getSubSystemNameByModularName(moduleName);
		if ((subSysName == null) || (subSysName.trim().length() == 0))
			...
			buffer.append(LicenseClientUtil
					.getModularAliasNameByModularName(subSysName)
					+ "[" + subSysName + "]");
		}
		buffer.append("\n");
		return buffer.toString();
	...
	public static final String constructLicenseMessageByModuleName(
			LicenseController lc, String moduleName) throws LicenseException {
		if ((lc == null) || (moduleName == null)) {
			return "";
		}
		StringBuffer buffer = new StringBuffer();
