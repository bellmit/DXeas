 ...
 *
 */
public class AROthertBillSupport implements IBillSupport {
	protected BigDecimal zero;
	private Context ctx;

	public AROthertBillSupport() {
...
//					}
	...
	//				billSta
					if(BillStatusEnum.SAVE.equals(status)){
						...
						pk = getInterface().submit(info);
						WsCommonUtil.updateBillNumber(ctx, "t_ap_otherbill", pk.toString(), number);
						info = getInterface().getOtherBillInfo(pk);
						try{
							if(BillStatusEnum.SUBMITED.equals(info.getBillStatus())){
								// 审核
