...
public class BaseBillImportUtils {
	...
	public static List setInfoByNumber(Context ctx, CoreBaseInfo info, List errorMsg) throws BOSException {
		return setDataInfoByNumber(ctx, info, errorMsg);
	}

	public static List setDataInfoByNumber(Context ctx, CoreBaseInfo info, List errorMsg) throws BOSException {
	 ...
	 * @throws BOSException
	 */
	public static boolean checkBillNumberExist(Context ctx,BOSObjectType bosType, String number)
			throws BOSException {

		ICoreBillBase iCoreBillBase = (ICoreBillBase) BOSObjectFactory.createBOSObject(ctx, bosType);
		
		CoreBillBaseCollection databaseCol = iCoreBillBase.getCoreBillBaseCollection((new StringBuilder(
				"select id,number where number='")).append(number).append("'").toString());
	...
	private static String getOql(DataBaseInfo lInfo, String number) {
		...
		if (lInfo instanceof CurrencyInfo)
			...
			sb.append("select id,number,name,isHasMaterialNumber,precision,destinationtype where number='").append(
					number).append("'");
		else
...
//			return;
		...
		if (databaseCol != null && databaseCol.size() > 0) {
			...
			lInfo = null;
			errorMsg.add((new StringBuilder("\u7F16\u7801")).append(number).append(
					" \u4E0D\u662F\u4E00\u4E2A\u6B63\u786E\u7684 ").append(getAliasName(ctx, info, key)).append(" .")
					.toString());
		}
		info.put(key, lInfo);
	}

	public static void setCollection(Context ctx, List errorMsg, Object obj) throws BOSException {
...
//		}
	...
	public static void setUserInfo(Context ctx, CoreBaseInfo info, List errorMsg, String key, Object obj)
		...
		UserCollection userCol = iuser.getUserCollection((new StringBuilder("select id,number ,name where number='"))
	...
	public static MultiMeasureUnitInfo getMultiMeasureUnit(Context ctx, MaterialInfo material, MeasureUnitInfo unit,
			List errorMsg) throws BOSException {
		MultiMeasureUnitInfo mmu = null;
		if (material != null && unit != null) {
			String key = (new StringBuilder(String.valueOf(material.getId().toString()))).append(
					unit.getId().toString()).toString();
			if (DataInfoCache.getInstance().containsKey(key)) {
				...
				if (mmu != null)
					...
					errorMsg.add((new StringBuilder("\u8BA1\u91CF\u5355\u4F4D")).append(unit.getNumber()).append(
							"\u4E0D\u5B58\u5728\u4E8E\u7269\u6599\u7684").append(material.getNumber()).append(
							"\u591A\u8BA1\u91CF\u5355\u4F4D\u7EC4\u91CC\u9762").toString());
			}
		}
		return mmu;
	...
	private static MultiMeasureUnitInfo getMultiMeasureUnitInfo(Context ctx, String materialId, String unitId)
		...
		String oql = (new StringBuilder(
				" select material.id, measureUnit.id, qtyPrecision,baseConvsRate where measureUnit.id ='")).append(
				unitId).append("' and material.id ='").append(materialId).append("'").toString();
	...
	public static MaterialCompanyInfoInfo getMaterialCompanyInfo(Context ctx, MaterialInfo material,
			CompanyOrgUnitInfo cou, List errorMsg) throws BOSException {
		MaterialCompanyInfoInfo materialcompanyInfo = null;
		if (material != null && cou != null) {
			String key = (new StringBuilder(String.valueOf(material.getId().toString())))
					.append(cou.getId().toString()).toString();
			if (DataInfoCache.getInstance().containsKey(key)) {
	...
	private static MaterialCompanyInfoInfo getMaterialCompanyInfo(Context ctx, String materialid, String couID)
		...
		MaterialCompanyInfoCollection con = MaterialCompanyInfoFactory.getLocalInstance(ctx)
				.getMaterialCompanyInfoCollection(viewInfo);
		if (con != null && con.size() >= 1)
			materialcompanyInfo = con.get(0);
		return materialcompanyInfo;
	}

	public static CompanyOrgUnitInfo getCompanyInfo(Context ctx, OrgUnitInfo orgInfo, int orgType) throws BOSException,
			EASBizException {
	...
	public static void getbillMetas(Context ctx, String bosType) {
		...
		for (int i = 0; i < properties.size(); i++) {
			...
			String sline = (new StringBuilder(String.valueOf(sp.getName()))).append("; ").append(sp.getAlias()).append(
					"; ").append(sp.getString("description")).toString();
			if (sp.isOwnProperty())
				sline = (new StringBuilder(String.valueOf(sline))).append("; ").append(
						sp.getDataType().getJavaTypeName()).toString();
			else
